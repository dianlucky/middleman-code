                        ...
                        <div v-for="(eachConversation, indexConversation) in conversations" :key="eachConversation.id">
                            <div v-if="eachConversation.user_sender_id === authId" class="chat-message d-flex mb-3 justify-content-start">
                                <div class="message-text" style="background-color: #dcf8c6; padding: 15px; border-radius: 15px; width: auto; min-width: 250px; max-width: 60%; position: relative; word-wrap: break-word; white-space: normal;">
                                    ...
                                    <span v-if="authRole == 'member'" class="message-check" style="position: absolute; top: 5px; right: -25px; font-size: 10px; color: #999;">
                    ...
                    <div class="input-group px-3 py-2 rounded">
                        ...
                        <div class="input-group-append">
                            <div class="btn-group dropup">
                                ...
                                <button v-if="authRole == 'member'" :disabled="! room" type="button" class="btn btn-secondary dropdown-toggle dropdown-toggle-split rounded" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></button>
                                <div v-if="authRole == 'member'" class="dropdown-menu rounded p-2">
                                    <a v-if="false" class="dropdown-item p-1" href="#"><small>Delete conversations</small></a>
                                    <a v-if="authRole == 'member'" @click="optionConversations ('hide')" class="dropdown-item p-1" href="#"><small>Hide conversations</small></a>
                                    <a v-if="authRole == 'member'" @click="optionConversations ('show')" class="dropdown-item p-1" href="#"><small>Show conversations</small></a>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        room ()
        {
            if (this.authId != this.room.admin.id) {
                ...
                .listen (".conversation.hidden", (item) => {
                    let indexConversation = this.conversations.findIndex (conversation => conversation.id === item.conversation.id);
                    this.conversations[indexConversation].is_admin_watchable = item.conversation.is_admin_watchable;
                })
                ...
                .listen (".conversation.hidden", (item) => {
                    let indexRoom = this.rooms.findIndex (room => room.id === item.conversation.room.id);
                    this.chooseRoom (indexRoom);
                })
                .listen (".conversation.shown", (item) => {
                    let indexRoom = this.rooms.findIndex (room => room.id === item.conversation.room.id);
                    this.chooseRoom (indexRoom);
